...
<nav>
  <ul class="pagination">
    ...
    <li :class="{'disabled': current == 1}"><a href="#" @click="setCurrent(1)"> {{$t('homeRecord.sep.homePage')}} </a></li>
    <li v-for="(p,index) in grouplist" :class="{'active': current == p.val}" :key="index">
      <a href="#" @click="setCurrent(p.val)"> {{ p.text }} </a>
    </li>
    <li :class="{'disabled': current == page}"><a href="#" @click="setCurrent(page)"> {{$t('homeRecord.sep.endPage')}} </a></li>
